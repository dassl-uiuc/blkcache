...
#include "thread_safe_lru/hash.h"

using PMRStorage = std::pmr::monotonic_buffer_resource;
...
struct ThreadSafeFixedKey {
  ThreadSafeFixedKey(const char* data, size_t size)
  {
    m_data = data;
    m_size = size;
  }
  ...
  bool operator==(const ThreadSafeFixedKey& other) const {
    size_t s = size();
    return s == other.size() && 0 == std::memcmp(data(), other.data(), s);
  }

  struct HashCompare {
    bool equal(const ThreadSafeFixedKey& j, const ThreadSafeFixedKey& k) const {
      return j == k;
    }

    size_t hash(const ThreadSafeFixedKey& k) const {
      return k.hash();
    }
  };
