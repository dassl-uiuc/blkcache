...
#include "thread_safe_lru/lru-cache.h"
#include "thread_safe_lru/string-key.h"
#include <limits>
#include <memory>
...
struct ThreadSafeScalableCache {
   ...
   *     count).
   */
  explicit ThreadSafeScalableCache(size_t maxSize, BlockCacheConfig block_cache_config_, size_t numShards = 0);
...
ThreadSafeScalableCache(size_t maxSize, BlockCacheConfig block_cache_config_, size_t numShards)
  : m_maxSize(maxSize), block_cache_config(block_cache_config_), m_numShards(numShards)
{
  if (m_numShards == 0) {
