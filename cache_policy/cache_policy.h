...
class RdmaManager {
public:
  RdmaManager(BlockCacheConfig block_cache_config_, std::shared_ptr<BlockDB> block_db_, std::size_t cache_size_) :
    block_cache_config(block_cache_config_), block_db(block_db_), cache_size(cache_size_)
  {
    // if (this->block_db_block_size != DEFAULT_PAGE_SIZE)
    // {
    //   panic("Block size needs to be page size {} != {}", this->block_db_block_size, DEFAULT_PAGE_SIZE);
    // }
    // void* mem{};
    // int res = posix_memalign(&mem, DEFAULT_PAGE_SIZE, this->block_db_num_entries);
    ...
    shared_memory = std::make_unique<infinity::memory::Buffer>(context.get(), 64 * 1024 * 1024 * sizeof(char));
    infinity::memory::RegionToken *buffer_token = shared_memory->createRegionToken();

    printf("Creating buffers to receive a message\n");
    internal_receiving_buffer = std::make_unique<infinity::memory::Buffer>(context.get(), 128 * sizeof(char));
    context->postReceiveBuffer(&*internal_receiving_buffer);


    auto server_thread = [&](auto machine_config)
    {
      machine_config.ip;
    };

    for (const auto& machine_config : this->block_cache_config.remote_machine_configs)
    {
      servers.push_back(std::thread(server_thread, machine_config));
      machine_config.ip;
    }

    auto num_connections = 2 * this->block_cache_config.remote_machine_configs.size();
...
class CachePolicy {
public:
  CachePolicy(BlockCacheConfig block_cache_config_, std::shared_ptr<BlockDB> block_db_, uint64_t cache_size_) :
    block_cache_config(block_cache_config_), block_db(block_db_), cache_size(cache_size_)
  {
